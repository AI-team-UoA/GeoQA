...
//				property.label = myQuestionNL.substring(property.begin,property.end);
				...
				logger.info("DBpedia (instance property) uri info {} label {}", s.getResource("uri").getURI(), property.label);
			}
...
//				System.out.println("Inside IP/PRI");
				System.out.println("***************** IP / PRI identified *****************");
				String spatialRelation = relationsList.get(0).get(0).relationFunction.toLowerCase();
				...
				for (Entity ents : instancesList.get(0)) {
					if (ents.uri.contains("dbpedia.org")) {
						if (ents.uri.contains("dbpedia.org")) {
							sparqlQ = "select ?property where { SERVICE <http://dbpedia.org/sparql> {  <" + ents.uri
									+ "> <"+propertyUri+"> ?property.} }";
						} else {
							sparqlQ = "select ?property where { SERVICE<> { <" + ents.uri
									+ "> owl:sameAs ?x. } SERVICE<>{ ?x <"+propertyUri+"> ?property. } } ";
						}
						Query q = new Query();
						q.query = sparqlQ;
						q.score = ents.linkCount;
